...
{
	*ppucMBTCPFrame = &Rx_Buffer[0];			//将TCP数据段（即MODBUS协议段）首地址赋给ppucMBTCPFrame
    *usTCPLength = LastResDataLen;				//LastResDataLen有效数据的长度
    /* Reset the buffer. */
    LastResDataLen = 0;
    return TRUE;
}
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    BOOL            bFrameSent = FALSE;
		...
		memcpy(Tx_Buffer, pucMBTCPFrame, usTCPLength);	// 复制待发送MODBUS段到TCP发送数据段内
// 	TCPTxDataCount = usTCPLength;        			// bytes to xfer，长度
// 	TCPTransmitTxBuffer();                   		// send last segment

		bFrameSent = S_tx_process(0, usTCPLength);
